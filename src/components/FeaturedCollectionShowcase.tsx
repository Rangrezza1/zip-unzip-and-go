...
import { FeaturedCollectionSection as FeaturedConfig, useThemeStore } from '@/stores/themeStore';
import ProductCard from './ProductCard';
import { Loader2 } from 'lucide-react';
import { CollectionSectionSettings } from '@/stores/themeStore';

const HeadingDecoration = ({ style }: { style: CollectionSectionSettings['headingStyle'] }) => {
  switch (style) {
    case 'underline':
      return <div className="w-12 h-0.5 bg-primary mx-auto mt-2" />;
    case 'accent-bar':
...
const FeaturedCollectionShowcase = ({ config }: Props) => {
  const animationsEnabled = useThemeStore((s) => s.theme.animationsEnabled);
  const { data, isLoading } = useCollectionProducts(config.collectionHandle, config.productLimit);
      ...
      <div className="container">
        ...
        {config.ctaText && (
          <div className="text-center mt-6">
            <a href={`/collections/${config.collectionHandle}`} className="cta-button-outline inline-block">
              {config.ctaText}
            </a>
          </div>
        )}
